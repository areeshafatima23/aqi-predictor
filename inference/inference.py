...
# ------------------------
# Load Best Model from MongoDB Registry
# ------------------------
def get_best_model():
    ...
    try:
        ...
        best = registry.find_one(
            {"target": "pm2_5"},
            sort=[("metrics.rmse", 1)]  # lowest RMSE first
        )
        if not best:
            raise RuntimeError("No PM2.5 model found in registry")

        model = joblib.load(os.path.join(best["model_path"], "model.pkl"))
        scaler = joblib.load(os.path.join(best["model_path"], "scaler.pkl"))
...
def run_inference():
    ...
    for i in range(3):
        pm25_day = recent_pm25 * (1 + 0.02 * i)  # simple daily growth
